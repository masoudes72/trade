...
def main_app():
    ...
    st.markdown("""
    <style>
        /* Main App Background and General Text */
        ...
        .stDataFrame tbody td a { color: #f0bb7d; } /* Table link color */
        /* Tab Styling */
        ...
        /* Button Styling */
        .stButton button {
            background-color: #e0a96d; color: #1b263b; font-weight: bold;
    ...
    PRESETS = {"Conservative": {'min_market_cap': 50e6, ...}, "Balanced": {'min_market_cap': 10e6, ...}, "Aggressive": {'min_market_cap': 1e6, ...}} # Presets kept short for brevity
    ...
    def load_or_fetch_data():
        ...
        with st.spinner("در حال واکشی تازه‌ترین داده‌ها... (این فرآیند فقط هر ۴ ساعت یکبار انجام می‌شود)"):
            ...
            while start_index <= fetch_limit:
                try:
                    ...
                    for c in lst:
                        q = c.get('quotes', [{}])[0]
                        rows.append({k: c.get(k) or q.get(v, None) for k, v in RAW_COLS.items()})
    ...
    def style_dataframe(df):
        ...
        formats = {k: v for k, v in {PROCESSED_COLS['price']: "${:,.4f}", ...}.items() if k in df.columns}
    ...
    # --- MAIN PAGE DISPLAY ---
    filter_params = {'min_market_cap': min_mc, ...}; df['volume_mc_ratio'] = df[PROCESSED_COLS['volume_24h']] / (df[PROCESSED_COLS['market_cap']] + 1e-9)
    filtered = df[(df[PROCESSED_COLS['market_cap']] >= filter_params['min_market_cap']) & ...]
    ...
    with tab2:
        ...
        if filtered.empty: st.warning("برای تحلیل ریتمیک، ابتدا باید کاندیداهایی در تب نتایج اولیه وجود داشته باشد.")
            ...
            with st.spinner("لطفا صبر کنید، تحلیل در حال انجام است..."):
                recs = filtered[[...]].to_dict("records"); progress_bar = st.progress(0.0, ...); status_text = st.empty()
            ...
            if results:
                ...
                df_r = pd.DataFrame(results); df_r = pd.merge(df_r, filtered[[...]], on=...); passed = df_r[df_r["pass"] == True].sort_values("score", ascending=False)
